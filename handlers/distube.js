        ...
        .on('addSong', async (queue, song) => {
            if (queue.textChannel) {
                const embed = new EmbedBuilder()
                    .setColor(0xDC92FF)
                    .setAuthor({ name: 'Song added successfully', iconURL: musicIcons.correctIcon, url: 'https://discord.gg/qX9ugahuqh' })
                    .setDescription(`**${song.name}**\n- Duration: **${song.formattedDuration}**\n- Added by: ${song.user}`)
                    .setFooter({ text: 'Arnab Player', iconURL: musicIcons.footerIcon })
                    .setTimestamp();
